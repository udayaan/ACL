...
int getnamed_user_or_group_perm(char* username, struct acl* meta, int flag) {
    char* list;
    if(flag==0){
...
void check_read_perm(uid_t ruid, gid_t gid, char* path) {
    ...
    int flag;
    flag = getnamed_user_or_group_perm(username,direc,0);
    if(flag==1){
        return;
    }
    ...
    for(int j=0;j<ngrps;++j) {
        ...
        gp = getgrgid(g);
        if(gp!=NULL){
            flag = getnamed_user_or_group_perm(gp->gr_name,direc,1);
            if(flag==1){
                return;
            }
        } 
